...
class OxfordWordMeaning:
        ...
        for result in results:
            ...
            for lexical_entry in lexical_entries:
                ...
                for entry in entries:
                    ...
                    audio_file = [pronunciation.get('audioFile') for pronunciation in pronunciations]
                    ...
                    for sense in senses:
                        ...
                        if subsenses:
                            sub_definitions = [subsense.get('definitions', []) for subsense in subsenses]
                            sub_short_definitions = [subsense.get('shortDefinitions', []) for subsense in subsenses]
                            sub_examples = [subsense.get('examples', []) for subsense in subsenses]
                        else:
                            sub_definitions = []
    ...
    def __format_meta_info(self, meta_word_info: dict) -> dict:
        ...
        ipa_indices = [i for i, x in enumerate(meta_word_info['phonetic_notation']) if x == 'IPA']
    ...
    def __format_meanings(self, word_meanings: list[dict]) -> list[dict]:
        ...
        for wm in word_meanings:
            ...
            sub_definitions = [sub_def for sublist in wm['sub_definitions'] for sub_def in sublist]

            # Extract text from sub_examples while maintaining sublist structure 
            sub_examples = [[example.get('text') for example in sublist] for sublist in wm['sub_examples']]
